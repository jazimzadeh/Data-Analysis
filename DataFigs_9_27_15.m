...
clear;clc;clf;
cd('/Users/Julien/Google Drive/Hudspeth/Data/Orange Room/2015-09-27.01/Ear 1/Cell 1')
...
%% Spontaneously oscillating cells - lasered
r = 7;
clf
logfile_name        = getfield(dir('*.log'),'name');
logfile_name_clean  = logfile_name(1:end-7);            % remove excess numbers from filename, for use in figure titles below
...
offset = 0;
for j = 22:41
    dc_offset = mean(struct_data(r).data(:,j));
    plot(t,struct_data(r).data(:,j)-dc_offset+offset)
    hold on
    offset = offset + 100;
end
