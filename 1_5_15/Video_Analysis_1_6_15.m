clear;clc;
cd('/Users/Julien/Google Drive/Hudspeth/Data/High-Speed Videos/1_6_15/')

all_x = zeros(320,18);
for event_number = 3:18
    clf
    ...
    %
    for wt = 1:12
        eval(['y' num2str(wt) '= ybars(1 + (320 * (' num2str(wt) '-1)) : 320*' num2str(wt) ');'])
    end
%     y1 = ybars(1:500,1);
%     y2 = ybars(501:1000,1);
%     y3 = ybars(1001:1500,1);
%     y4 = ybars(1501:2000,1);
    y_average = (y1 + y2 + y3 + y4+y5+y6+y7+y8+y9+y10+y11+y12) / 12; 

    %
     for wt = 1:12
        eval(['x' num2str(wt) '= xbars(1 + (320 * (' num2str(wt) '-1)) : 320*' num2str(wt) ');'])
    end
%     x1 = xbars(1:500,1);
%     x2 = xbars(501:1000,1);
%     x3 = xbars(1001:1500,1);
%     x4 = xbars(1501:2000,1);
    x_average = (x1 + x2 + x3 + x4+x5+x6+x7+x8+x9+x10+x11+x12) / 12; 
    
    all_x(:,event_number) = x_average;          % store the averaged x displacement for each event in its own column
end
