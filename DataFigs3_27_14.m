...
cd('/Users/Julien/Google Drive/Hudspeth/Data/Orange Room/2014-03-27.01/Ear 1/Cell 1')
...
for n = 3:8
m = n-2;   % Correcting for the fact that rec. 1 starts on the third rec. of the day
...
title(['Recording ' num2str(m)])
subplot(2,1,2)
plot(t,eval(['struct_data.data' num2str(n) '.data(:,17:21)']))
ylabel('pA'); xlabel('Seconds')
saveas(gcf,strcat('figure',num2str(m),'.pdf'));
clf
end

% weird, rec's 9 and 10 from today have potnetial mech i's, but they are
% timestamped from before the other runs.
