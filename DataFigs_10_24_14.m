...
clear;clc;clf;
cd('/Users/Julien/Google Drive/Hudspeth/Data/Orange Room/2014-10-24.01/Ear 1/Cell 1')
...
%%  Plot ionto stim, displacement, & plots of displacements as a function of ionto
...
for r = 1:12      % Select the record numbers to plot
    logfile_name = getfield(dir('*.log'),'name');
    logfile_name_clean = logfile_name(1:end-7); % remove excess numbers from filename, for use in figure titles below
    logfile = importdata(logfile_name);
    clf
...
r=11;
     i= 2*wavetrain+1
        dc = mean(struct_data(r).data(1:100,i));
        plot(t(13200:15600),-struct_data(r).data(13200:15600,i)-dc+offset)
        hold all
        offset = offset+120;
...
%% Plot records 13 to end (fatigue experiments and other short records)
clf
logfile_name = getfield(dir('*.log'),'name');
logfile_name_clean = logfile_name(1:end-7); % remove excess numbers from filename, for use in figure titles below
logfile = importdata(logfile_name);
