...
clear;clc;clf
cd('/Users/Julien/Google Drive/Hudspeth/Data/High-Speed Videos/8_20_15/')
...
clf
for r = 2:11
    plot(record(2).time,record(r).CoM_avg + record(r).distance)
    hold on
end
...
for i = [2 4 6 9 11]        % these are the events in which the bundle was in the laser beam
    event_HB1(counter) = i-1;
    delta_HB1(counter) = record(i).delta;
    counter = counter + 1;
    plot(i-1, record(i).delta,'.','MarkerSize',24);
    hold on
end
%axis([0 6 0.0 0.12])
ylabel('um')
...
for i = [2:11]
    ...
    c_factor = decayed/record(2).delta; % Use record 2 as the baseline
...
saveas(gcf,strcat('Decay_Correction_8_20_15_HB1','fig'));
saveas(gcf,strcat('Decay_Correction_8_20_15_HB1','eps'),'epsc');

%% Plot decay of response as a function of event number, HB2
clf
counter = 1;
subplot(4,1,1:2)
for i = [13 15 17]        % these are the events in which the bundle was in the laser beam
    event_HB2(counter) = i-12;
    delta_HB2(counter) = record(i).delta;
    counter = counter + 1;
    plot(i-12, record(i).delta,'.','MarkerSize',24);
    hold on
end
axis([0 6 0 0.06])
ylabel('um')
...
% Now plot corrected records from inside the beam 
for i = [13:18]
    decayed = p_HB2(1) * (i-12) + p_HB2(2);
    c_factor = decayed/record(13).delta;
    record(i).corr_delta = record(i).delta/c_factor;    % Create a corr_delta field in each record, with the corrected amplitude
end

subplot(4,1,3:4)
for i = [13 15 17]       % these are the events in which the bundle was in the laser beam
    h1 = plot(i-12, record(i).delta,'k.','MarkerSize',24);
    hold on
    h2 = plot(i-12, record(i).corr_delta,'r.','MarkerSize',24);
end
xlabel('Event #')
...
saveas(gcf,strcat('Decay_Correction_8_20_15_HB2','fig'));
saveas(gcf,strcat('Decay_Correction_8_20_15_HB2','eps'),'epsc');
...
subplot(4,1,1:2)
for i = [19 21 23 26]  
    event_HB3(counter) = i-18;
    delta_HB3(counter) = record(i).delta;
    counter = counter + 1;
    plot(i-18, record(i).delta,'.','MarkerSize',24);
    hold on
end
axis([0 10 0 0.1])
ylabel('um')
...
for i = [19:26]
    decayed = p_HB3(1) * (i-18) + p_HB3(2);
    c_factor = decayed/record(19).delta;        % compare the values gotten fit to the original (non-decayed) to get correction factor
    record(i).corr_delta = record(i).delta/c_factor;
end
subplot(4,1,3:4)
for i = [19 21 23 26]        % these are the events in which the bundle was in the laser beam
    h1 = plot(i-18, record(i).delta,'k.','MarkerSize',24);
    hold on
    h2 = plot(i-18, record(i).corr_delta,'r.','MarkerSize',24);
end
xlabel('Event #')
ylabel('Displacement (um)')
...
saveas(gcf,strcat('Decay_Correction_8_20_15_HB3','fig'));
saveas(gcf,strcat('Decay_Correction_8_20_15_HB3','eps'),'epsc');
...
title('Displacement v. Distance from laser beam, 8/20/15 HB1')
...
saveas(gcf,strcat('Decay_Corrected_v_Uncorrected_8_20_15_HB1','fig'));
saveas(gcf,strcat('Decay_Corrected_v_Uncorrected_8_20_15_HB1','eps'),'epsc');
...
axis([-2 30 0 130])
ylabel('Laser-evoked displacement (nm)')
title('Displacement v. Distance from laser beam, 8/20/15 HB2')
...
saveas(gcf,strcat('Decay_Corrected_v_Uncorrected_8_20_15_HB2','fig'));
saveas(gcf,strcat('Decay_Corrected_v_Uncorrected_8_20_15_HB2','eps'),'epsc');
...
% error
clf
set(0, 'DefaultAxesColorOrder', ametrine(8));
subplot(2,1,1)
for i = 19:26
   plot(record(i).distance,record(i).delta*1000,'.','Markersize',24)
   hold all
...
saveas(gcf,strcat('Decay_Corrected_v_Uncorrected_8_20_15_HB3','fig'));
saveas(gcf,strcat('Decay_Corrected_v_Uncorrected_8_20_15_HB3','eps'),'epsc');

%% All HBs - Calculate what % of maximum amplitude each event represents, then plot it
for i = [2 3 5 10]
   record(i).corr_delta_percent = record(i).corr_delta / record(2).corr_delta; 
end
subplot(3,1,1)
for i = [2 3 5 10]
...
for i = [13 14 16]
   record(i).corr_delta_percent = record(i).corr_delta / record(13).corr_delta; 
end
subplot(3,1,2)
for i = [13 14 16]
   plot(record(i).distance , record(i).corr_delta_percent ,'.k','Markersize',24)
...
title('9/20/15 - Fraction of initial displacement v. Distance')
xlabel('Distance (um)')
ylabel('Fraction of initial displacement')

saveas(gcf,strcat('Normalized_Displacements_8_20_15_HBs1_2_3','fig'));
saveas(gcf,strcat('Normalized_Displacements_8_20_15_HBs1_2_3','eps'),'epsc');

%% Overlay to look at kinetics - HB1
clf
subplot_tight(3,1,1,[0.05 0.1])
set(0, 'DefaultAxesColorOrder', ametrine(4));
h1 = plot(record(2).time(5:end), record(2).CoM_avg(5:end)/min(record(2).CoM_avg(10:end)),'linewidth',2);
hold all
...
sprintf('%d um, %d nm', round(record(10).distance), round(record(10).corr_delta*1000)),'location','northwest')
title('8/20/15 - HB1')
axis([0 100 -0.2 1.1])
...
h1 = plot(record(2).time(5:end), record(2).CoM_avg(5:end)/min(record(2).CoM_avg(10:end)),'linewidth',2);
    hold all
h2 = plot(record(4).time(5:end), record(4).CoM_avg(5:end)/min(record(4).CoM_avg(10:end)),'linewidth',2);
h3 = plot(record(6).time(5:end), record(6).CoM_avg(5:end)/min(record(6).CoM_avg(10:end)),'linewidth',2);
h4 = plot(record(9).time(5:end), record(9).CoM_avg(5:end)/min(record(9).CoM_avg(10:end)),'linewidth',2);
h5 = plot(record(11).time(5:end), record(11).CoM_avg(5:end)/min(record(11).CoM_avg(10:end)),'linewidth',2);
...
set(0, 'DefaultAxesColorOrder', ametrine(4));
h1 = plot(record(6).time(5:end), record(6).CoM_avg(5:end)/min(record(6).CoM_avg(10:end)),'linewidth',2);
    hold all
h2 = plot(record(7).time(5:end), record(7).CoM_avg(5:end)/min(record(7).CoM_avg(10:end)),'linewidth',2);
h3 = plot(record(8).time(5:end), record(8).CoM_avg(5:end)/min(record(8).CoM_avg(10:end)),'linewidth',2);
h4 = plot(record(9).time(5:end), record(9).CoM_avg(5:end)/min(record(9).CoM_avg(10:end)),'linewidth',2);
legend([h1 h2 h3 h4], ...
sprintf('%d um, 4V, %d nm',     round(record(6).distance), round(record(6).corr_delta*1000)), ...
sprintf('%d um, 1.5V, %d nm',   round(record(7).distance), round(record(7).corr_delta*1000)),...
sprintf('%d um, 2V, %d nm',     round(record(8).distance), round(record(8).corr_delta*1000)),...
sprintf('%d um, 4V, %d nm',     round(record(9).distance), round(record(9).corr_delta*1000)),...
'location','northwest');
xlabel('Time (ms)')
axis([0 100 -0.2 1.1])

saveas(gcf,strcat('Overlaid_Kinetics_8_20_15_HB1','fig'));
saveas(gcf,strcat('Overlaid_Kinetics_8_20_15_HB1','eps'),'epsc');

%% Overlay to look at kinetics - HB2
clf
subplot_tight(3,1,1,[0.05 0.1])
set(0, 'DefaultAxesColorOrder', ametrine(3));
h1 = plot(record(13).time(5:end),  -record(13).CoM_avg(5:end)/max(-record(13).CoM_avg(10:end)),'linewidth',2);
hold all
h2 = plot(record(14).time(5:end),  -record(14).CoM_avg(5:end)/max(-record(14).CoM_avg(10:end)),'linewidth',2);
h3 = plot(record(16).time(5:end), -record(16).CoM_avg(5:end)/max(-record(16).CoM_avg(10:end)),'linewidth',2);
legend([h1 h2 h3], ...
...
h1 = plot(record(13).time(5:end), -record(13).CoM_avg(5:end)/max(-record(13).CoM_avg(10:end)),'linewidth',2);
    hold all
h2 = plot(record(15).time(5:end), -record(15).CoM_avg(5:end)/max(-record(15).CoM_avg(10:end)),'linewidth',2);
...
subplot_tight(3,1,3,[0.05 0.1])
h1 = plot(record(17).time(5:end), -record(17).CoM_avg(5:end)/max(-record(17).CoM_avg(10:end)),'linewidth',2);
    hold all
h2 = plot(record(18).time(5:end), -record(18).CoM_avg(5:end)/max(-record(18).CoM_avg(10:end)),'linewidth',2);
legend([h1 h2], ...
sprintf('%d um, 4V, %d nm',     round(record(17).distance), round(record(17).corr_delta*1000)),...
sprintf('%d um, 2.5V, %d nm',   round(record(18).distance), round(record(18).corr_delta*1000)), ...
'location','northwest');
xlabel('Time (ms)')
axis([0 100 -0.4 1.2])

saveas(gcf,strcat('Overlaid_Kinetics_8_20_15_HB2','fig'));
saveas(gcf,strcat('Overlaid_Kinetics_8_20_15_HB2','eps'),'epsc');

%% Overlay to look at kinetics - HB3
clf
subplot_tight(3,1,1,[0.05 0.1])
set(0, 'DefaultAxesColorOrder', ametrine(3));
h1 = plot(record(19).time(5:end),  record(19).CoM_avg(5:end)/max(record(19).CoM_avg(10:end)),'linewidth',2);
hold all
...
h1 = plot(record(19).time(5:end), record(19).CoM_avg(5:end)/max(record(19).CoM_avg(10:end)),'linewidth',2);
    hold all
h2 = plot(record(21).time(5:end), record(21).CoM_avg(5:end)/max(record(21).CoM_avg(10:end)),'linewidth',2);
h3 = plot(record(23).time(5:end), record(23).CoM_avg(5:end)/max(record(23).CoM_avg(10:end)),'linewidth',2);
h4 = plot(record(26).time(5:end), record(26).CoM_avg(5:end)/max(record(26).CoM_avg(10:end)),'linewidth',2);
legend([h1 h2 h3 h4 ], ...
sprintf('%d um, %d nm', round(record(19).distance), round(record(19).corr_delta*1000)),...
...
h4 = plot(record(26).time(5:end), record(26).CoM_avg(5:end)/max(record(26).CoM_avg(10:end)),'linewidth',2);
legend([h1 h2 h3 h4 ], ...
sprintf('%d um, 4V, %d nm',     round(record(23).distance), round(record(23).corr_delta*1000)),...
sprintf('%d um, 1.5V, %d nm',   round(record(24).distance), round(record(24).corr_delta*1000)), ...
sprintf('%d um, 2.5V, %d nm',   round(record(25).distance), round(record(25).corr_delta*1000)),...
sprintf('%d um, 4V, %d nm',     round(record(26).distance), round(record(26).corr_delta*1000)),...
'location','northwest');
xlabel('Time (ms)')
axis([0 100 -0.4 1.2])

saveas(gcf,strcat('Overlaid_Kinetics_8_20_15_HB3','fig'));
saveas(gcf,strcat('Overlaid_Kinetics_8_20_15_HB3','eps'),'epsc');

%% Put the normalized data for all 3 bundles in one variable, so I can combine it with 8/21/15
clf
counter = 1;
for i = [2 3 5 10 13 14 16 19 20 22]
...
clf
hold on
h1 = plot(ag(1:4,1),ag(1:4,2),'.','color',[1 0 0],'markersize',28);
hold on
h2 = plot(ag(5:7,1),ag(5:7,2),'.','color',[1 0.6 0],'markersize',28);
h3 = plot(ag(8:10,1),ag(8:10,2),'.','color',[1 0.9 0],'markersize',28);
h4 = plot(ag(11:14,1),ag(11:14,2),'.','color',[0 0.9 0],'markersize',28);
h5 = plot(ag(15:18,1),ag(15:18,2),'.','color',[0 0.4 1],'markersize',28);
h6 = plot(ag(19:20,1),ag(19:20,2),'.','color',[0.6 0 1],'markersize',28);
h7 = plot(ag(21:24,1),ag(21:24,2),'.','color',[0.6 0 1],'markersize',28);

% Add the laser spread plots:
h8 = plot(x(435:end)-188.2,1*(aggregate_x((435:end),12)/max(aggregate_x(:,12))),'Linewidth',2,'color',[0 0 0]);
hold on
h9 = plot(x(435:end)-188.2,1*(aggregate_x(435:end,38)/max(aggregate_x(:,38))),'Linewidth',2,'color',[0.4 0.4 0.4]);
legend([h1 h2 h3 h4 h5 h6 h7 h8 h9],'8/20 HB1','8/20 HB2','8/20 HB3','8/21 HB1','8/21 HB2','8/21 HB3','8/21 HB4','Fluorescein','DiA')
...
axis([-10 70 0 1.5])
cd('/Users/Julien/Google Drive/Hudspeth/Data/High-Speed Videos/8_20_15/')
saveas(gcf,strcat('8_20_15_and_8_21_15_and_laserDecay','fig'));
saveas(gcf,strcat('8_20_15_and_8_21_15_and_laserDecay','eps'),'epsc');

%% PLOT DATA FROM 8/20 AND 8/21 with lines
cd('/Users/Julien/Google Drive/Hudspeth/Data/High-Speed Videos/7_9_15/')
load('Hamamatsu_variables.mat')
...
h1 = plot(ag(1:4,1),ag(1:4,2),'linewidth',2);
hold all
h2 = plot(ag(5:7,1),ag(5:7,2),'linewidth',2);
h3 = plot(ag(8:10,1),ag(8:10,2),'linewidth',2);
h4 = plot(ag(11:14,1),ag(11:14,2),'linewidth',2);
h5 = plot(ag(15:18,1),ag(15:18,2),'linewidth',2);
h6 = plot(ag(19:20,1),ag(19:20,2),'linewidth',2);
h7 = plot(ag(21:24,1),ag(21:24,2),'linewidth',2);
...
axis([-10 70 0 1.5])
cd('/Users/Julien/Google Drive/Hudspeth/Data/High-Speed Videos/8_20_15/')
saveas(gcf,strcat('8_20_15_and_8_21_15_and_laserDecay_lines','fig'));
saveas(gcf,strcat('8_20_15_and_8_21_15_and_laserDecay_lines','eps'),'epsc');

%% PLOT DATA FROM 7/9, 7/21, 8/20, AND 8/21
cd('/Users/Julien/Google Drive/Hudspeth/Data/High-Speed Videos/7_9_15/')
load('Hamamatsu_variables.mat')
...
legend([h1 h2 h3 h4 h5 h6 h7 h8 h9 h10 h11 h12 h13],'8/20 HB1','8/20 HB2','8/20 HB3','8/21 HB1','8/21 HB2','8/21 HB3','8/21 HB4','7/9 HB1','7/9 HB2','7/9 HB3','7/21 HB1','7/21 HB2','7/21 HB3','Fluorescein','DiA');
...
axis([-10 70 0 1.5])
cd('/Users/Julien/Google Drive/Hudspeth/Data/High-Speed Videos/8_20_15/')
saveas(gcf,strcat('7_9_and_7_21_and_8_20_and_8_21_2015_and_laserDecay','fig'));
saveas(gcf,strcat('7_9_and_7_21_and_8_20_and_8_21_2015_and_laserDecay','eps'),'epsc');
