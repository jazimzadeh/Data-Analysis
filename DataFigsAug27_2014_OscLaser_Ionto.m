...
for r = 1:length(fileList2)       % Select the record numbers to plot
    ...
    for i= (wavetrain+2) : (2*wavetrain+1)
        ...
        if laser_voltage ~= 0
            line([laser_delay; (laser_delay + laser_duration)],[-30; -30],'color','m','linewidth',5)
            textXpos = laser_delay + 0.3*laser_duration;
            text(textXpos,-15,num2str(laser_voltage),'color','m','fontsize',20)
        end
