function [ksf,lambdaxx,lambdadx,kf,lambdasf,R2,CI,fitA,fitB,iterations,tel] = fibercalcPSDLV(FreqAll,PSDAll,minfreq,maxfreq,maxiter)
%
% ----------------------
...
fitopts = fitoptions('Algorithm','Levenberg-Marquardt','Method','NonlinearLeastSquares','TolFun',1e-10,'TolX',1e-10,'MaxFunEvals',4000,'MaxIter',4000,'Robust','Off');
...
while rt1 == 0 || rt2 == 0 || sign(Acoeffs(1)) == -1 || sign(Acoeffs(2)) == -1
    ...
    fitopts = fitoptions('Algorithm','Levenberg-Marquardt','Method','NonlinearLeastSquares','TolFun',1e-10,'TolX',1e-10,'MaxFunEvals',4000,'MaxIter',4000,'Robust','Off','Exclude',outliers);
    [fitA,gofA] = fit(FreqAll(qmin:qmax),PSDAll(qmin:qmax),fit1,fitopts);
...
lambdasfB = 4.114/(pi^2*Bcoeffs(1));    % pN新/nm    
