...
for r = 3      % Select the record numbers to plot
    clf
    ...
    for i= (wavetrain+2) : (2*wavetrain+1)
        ...
        plot(t(1:100:end),-struct_data(r).data((1:100:end),i)-dc+offset)
        hold all
        offset = offset+90;
    end
...
for r = 5      % Select the record numbers to plot
    logfile_name = getfield(dir('*.log'),'name');
    logfile_name_clean = logfile_name(1:end-7); % remove excess numbers from filename, for use in figure titles below
    logfile = importdata(logfile_name);
    clf
...
%%
for r = 6      % Select the record numbers to plot
    logfile_name = getfield(dir('*.log'),'name');
    logfile_name_clean = logfile_name(1:end-7); % remove excess numbers from filename, for use in figure titles below
    logfile = importdata(logfile_name);
    clf
...
%%
for r = 8      % Select the record numbers to plot
    logfile_name = getfield(dir('*.log'),'name');
    logfile_name_clean = logfile_name(1:end-7); % remove excess numbers from filename, for use in figure titles below
    logfile = importdata(logfile_name);
    clf
